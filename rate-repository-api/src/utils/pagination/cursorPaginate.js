...
const getValidLimitOrFail = ({ first, last }) => {
  ...
  if (limit < 0 || limit > 100)
    throw new Error('Limit must be greater than 0 and less or equal to 100');
...
const cursorPaginate = async (
  ...
  if (before) orderBy = reverseOrderBy(orderBy);
  ...
  const paginateQuery = cursorQuery.clone().limit(limit).orderBy(orderBy);
